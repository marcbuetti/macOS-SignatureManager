...
private extension UpdateManager {
    ...
    func loadJobs() -> [SignatureJob] {
        var list: [SignatureJob] = []
        var missingHTMLNames: [String] = []
        var missingSignatureFiles: [String] = []

        let count = UserDefaults.standard.integer(forKey: "app.signatures.count")
        ...
        for i in 1...count {
            guard
                ...
                Logger.shared.log(position: "UPDATE.loadJobs", type: "WARNING", content: "Skipping slot #\(i): missing id/name/html keys or empty values")
                continue
            }

            let htmlPath = (html as NSString).expandingTildeInPath
            if FileManager.default.fileExists(atPath: htmlPath) == false {
                missingHTMLNames.append(name)
                Logger.shared.log(position: "VALIDATE.loadJobs", type: "CRITICAL", content: "Missing HTML file for signature \"\(name)\" @ \(htmlPath)")
                //self.notifyUpdateFailure(body: "Update failed: Missing HTML file for \(name)")
                continue
            }
            ...
            if FileManager.default.fileExists(atPath: signaturePath) == false {
                ...
                Logger.shared.log(position: "VALIDATE.loadJobs", type: "CRITICAL", content: "Missing .mailsignature for id=\(id) (name=\"\(name)\") @ \(signaturePath)")
                //self.notifyUpdateFailure(body: "Update failed: Missing .mailsignature for \(name)")
                continue
            }

            list.append(SignatureJob(id: id, htmlPath: htmlPath, name: name))
        }
        ...
        if !missingHTMLNames.isEmpty {
            ...
            //self.notifyUpdateFailure(body: "Update failed: Missing HTML files for: \(missingHTMLNames.joined(separator: ", "))")
        }
        if !missingSignatureFiles.isEmpty {
            ...
            Logger.shared.log(position: "VALIDATE.loadJobs", type: "CRITICAL", content: msg)
            //self.notifyUpdateFailure(body: "Update failed: Missing .mailsignature for: \(missingSignatureFiles.joined(separator: ", "))")
        }
